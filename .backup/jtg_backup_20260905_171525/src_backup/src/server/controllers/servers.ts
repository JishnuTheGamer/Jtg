...
export const getServerStats = async (req: Request, res: Response) => {
  ...
  if (server.containerId) {
    ...
    res.json({
      ...
      startedAt,
      uptimeSeconds,
      limitRam: Number(server.ram) > 0 ? Number(server.ram) * 1024 : 0,
      limitCpu: Number(server.cpu) || 0,
      limitDisk: Number(server.disk) || 0
    });
  } else {
    res.json({
      cpu: 0,
...
export const createServer = async (req: Request, res: Response) => {
  ...
  let { name, description, ram, port, version, theme, cpu, cpuLimit, disk, diskLimit, owner, ownerId, ipAlias, type, nodeId, runtimeType, expiresAt } = req.body;
  const settings = await readJSON("settings.json") || {};
  if (process.env.PORT !== "3000") {
    runtimeType = settings.defaultRuntime || process.env.DEFAULT_RUNTIME || "docker";
  }
  if (!name || port === undefined || port === null) {
    return res.status(400).json({ error: "Missing required fields (name, port)" });
  }
  ...
  const serverData = {
    ...
    ram: Number.isFinite(Number(ram)) ? Number(ram) : 0,
    cpu: Number.isFinite(Number(cpu ?? cpuLimit)) ? Number(cpu ?? cpuLimit) : 0,
    disk: Number.isFinite(Number(disk ?? diskLimit)) ? Number(disk ?? diskLimit) : 0,
    port: Number(port),
    ipAlias: ipAlias || "",
...
export const startServer = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (server.expiresAt && new Date(server.expiresAt).getTime() <= Date.now()) {
      return res.status(403).json({ error: "Server deployment has expired" });
    }
    ...
    // Remove stale Minecraft world locks left by an interrupted process.
    try {
      ...
      for (const lockFile of lockFiles) {
        if (await fs.pathExists(lockFile)) {
          try {
            ...
            return res.status(500).json({ error: `Unable to clean stale ${path.basename(lockFile)}` });
          }
        }
      }
