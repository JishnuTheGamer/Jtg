...
export const startLocalServer = async (id: string, serverData: any) => {
  ...
  if (type === "nodejs" || type === "node") {
    ...
    const configuredRamGb = Number(serverData.ram);
    const memory = configuredRamGb > 0 ? Math.round(configuredRamGb * 1024) : 0;
    ...
    if (serverData.startupCommand && serverData.startupCommand.trim()) {
      ...
      const javaArgs = memory > 0 ? [`-Xms${Math.max(128, memory)}M`, `-Xmx${Math.max(128, memory)}M`] : [];
      child = spawn(javaBin, [...javaArgs, "-Djline.terminal=jline.UnsupportedTerminal", "-jar", "server.jar", "nogui", "--nojline"], {
        cwd: serverPath,
        stdio: ["pipe", "pipe", "pipe"]
      });
    }
