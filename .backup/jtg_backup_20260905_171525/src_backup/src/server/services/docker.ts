...
// Sandbox mode is opt-in for development only. It must never be selected just
// because ENABLE_DOCKER was omitted from the environment.
export const isSandbox = process.env.SANDBOX_MODE === "true";
...
const appendSandboxLog = async (serverId: string, message: string) => {
  const logPath = path.join(process.cwd(), ".data", "servers", serverId, "panel.log");
...
export const createServerContainer = async (serverData: any, nodeId?: string) => {
  ...
  const buildContainerOptions = (img: string) => {
    ...
    const ram = Number(serverData.ram);
    const cpu = Number(serverData.cpu);
    if (Number.isFinite(ram) && ram > 0) hostConfig.Memory = Math.round(ram * 1024 * 1024);
    if (Number.isFinite(cpu) && cpu > 0) hostConfig.NanoCpus = Math.round(cpu * 10_000_000);
...
export const startContainer = async (containerId: string, nodeId?: string) => { console.log(`[startContainer] id=${containerId}, nodeId=${nodeId}`);
  ...
  if (isNodeSandbox(nodeId)) {
    ...
    const message = `[System] Server started (Sandbox Mode).\r\n`;
    await appendSandboxLog(id, message);
    panelEvents.emit("log", id, message);
    return;
  }
...
export const stopContainer = async (containerId: string, nodeId?: string) => {
  ...
  if (isNodeSandbox(nodeId)) {
    ...
    const message = `[System] Server stopped (Sandbox Mode).\r\n`;
    await appendSandboxLog(id, message);
    panelEvents.emit("log", id, message);
    return;
  }
...
export const killContainer = async (containerId: string, nodeId?: string) => {
  ...
  if (isNodeSandbox(nodeId)) {
    ...
    mockState[id] = false;
    delete mockStartedAt[id];
    const message = `[System] Server forcefully killed (SIGKILL).\r\n`;
    await appendSandboxLog(id, message);
    panelEvents.emit("log", id, message);
    return;
  }
...
export const restartContainer = async (containerId: string, nodeId?: string) => {
  ...
  if (isNodeSandbox(nodeId)) {
    ...
    mockState[id] = true;
    mockStartedAt[id] = new Date().toISOString();
    const message = `[System] Server restarted (Sandbox Mode).\r\n`;
    await appendSandboxLog(id, message);
    panelEvents.emit("log", id, message);
    return;
  }
...
export const getContainerLogs = async (containerId: string, nodeId?: string): Promise<string> => {
  const docker = await getDocker(nodeId);
  if (isNodeSandbox(nodeId)) {
    const logPath = path.join(process.cwd(), ".data", "servers", containerId.replace("mock-container-id-", ""), "panel.log");
    try {
...
export const sendContainerCommand = async (containerId: string, command: string, nodeId?: string) => {
  ...
  if (isNodeSandbox(nodeId)) {
    const id = containerId.replace("mock-container-id-", "");
    const message = `> ${command}\r\n[System] Sandbox mode: command accepted.\r\n`;
    await appendSandboxLog(id, message);
    panelEvents.emit("log", id, message);
    return;
  }
