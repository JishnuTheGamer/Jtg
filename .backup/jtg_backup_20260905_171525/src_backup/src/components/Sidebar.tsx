...
import { Server, LayoutDashboard, Plus, LogOut, X, Settings, Key, User, Activity, Box, Database, Menu } from "lucide-react";
...
export function Sidebar({ onClose, isCollapsed, toggleCollapse }: { onClose?: () => void, isCollapsed?: boolean, toggleCollapse?: () => void }) {
  ...
  if ((user?.role === "admin" || user?.role === "owner")) {
    ...
    links.push({ name: "Database", path: "/database", icon: <Database size={20} /> });
