...
export default function ServerConsole({ serverId, server }: ServerConsoleProps) {
  ...
  const appendLogLines = (incoming: string[]) => {
    setLogs((previous) => {
      const next = [...previous];
  ...
  useEffect(() => {
    ...
    let alive = true;
    axios.get<{ logs: string }>(`/api/servers/${serverId}/logs`).then(({ data }) => {
      if (!alive || !data?.logs) return;
      appendLogLines(data.logs.split(/\r?\n/));
    }).catch(() => {});
