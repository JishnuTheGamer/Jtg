...
router.get("/:id/logs", async (req, res) => {
  try {
    ...
    const server = servers.find((item: any) => item.id === req.params.id);
    const user = (req as any).user;
    if (!server) return res.status(404).json({ error: "Server not found" });
    if (user.role !== "admin" && user.role !== "owner" && server.owner !== user.id) {
      return res.status(403).json({ error: "Forbidden" });
    }
    return res.json({ logs: await getServerRuntimeLogs(server) });
  } catch (error: any) {
    return res.status(500).json({ error: error.message || "Failed to load server logs" });
  }
});
