...
export default function AdminSettingsPage(): React.ReactElement {
  ...
  const { 
    panelName, panelLogo, panelBackgroundImage, panelBackgroundBlur, blueprintMode, blueprintOpacity, blueprintSize,
  ...
  const [newBlueprintMode, setNewBlueprintMode] = useState(blueprintMode !== false);
  const [newBlueprintOpacity, setNewBlueprintOpacity] = useState(Number(blueprintOpacity ?? 0.12));
  const [newBlueprintSize, setNewBlueprintSize] = useState(Number(blueprintSize ?? 48));
  ...
  useEffect(() => {
    ...
    setNewTheme(theme);
    setNewBlueprintMode(blueprintMode !== false);
    setNewBlueprintOpacity(Number(blueprintOpacity ?? 0.12));
    setNewBlueprintSize(Number(blueprintSize ?? 48));
  ...
  }, [defaultRuntime, panelName, panelBackgroundImage, enablePlayit, enableTutorial, enableLoginAnimation, enableRegistration, theme, setTheme, blueprintMode, blueprintOpacity, blueprintSize, enableGoogleLogin, firebaseApiKey, firebaseAuthDomain, firebaseProjectId, firebaseStorageBucket, firebaseMessagingSenderId, firebaseAppId]);
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-4 pt-6 border-t border-border-subtle mt-6">
                    <div className="flex items-center justify-between gap-4">
                      <div>
                        <label className="block text-sm font-medium text-foreground">Blueprint overlay</label>
                        <p className="text-xs text-muted-foreground mt-1">Use a configurable technical grid across the panel background.</p>
                      </div>
                      <button type="button" role="switch" aria-checked={newBlueprintMode} onClick={async () => {
                        const value = !newBlueprintMode;
                        setNewBlueprintMode(value);
                        await axios.put("/api/system/settings", { blueprintMode: value });
                        await fetchSettings();
                      }} className={`relative h-6 w-11 rounded-full transition-colors ${newBlueprintMode ? "bg-theme-600" : "bg-zinc-700"}`}>
                        <span className={`absolute top-1 h-4 w-4 rounded-full bg-white transition-transform ${newBlueprintMode ? "translate-x-6" : "translate-x-1"}`} />
                      </button>
                    </div>
                    <label className="block text-xs font-bold text-muted-foreground uppercase tracking-widest">Grid opacity ({Math.round(newBlueprintOpacity * 100)}%)</label>
                    <input type="range" min="0" max="0.5" step="0.01" value={newBlueprintOpacity} onChange={(e) => setNewBlueprintOpacity(Number(e.target.value))} onMouseUp={async () => { await axios.put("/api/system/settings", { blueprintOpacity: newBlueprintOpacity }); await fetchSettings(); }} className="w-full accent-theme-600" />
                    <label className="block text-xs font-bold text-muted-foreground uppercase tracking-widest">Grid size ({newBlueprintSize}px)</label>
                    <input type="range" min="16" max="160" step="4" value={newBlueprintSize} onChange={(e) => setNewBlueprintSize(Number(e.target.value))} onMouseUp={async () => { await axios.put("/api/system/settings", { blueprintSize: newBlueprintSize }); await fetchSettings(); }} className="w-full accent-theme-600" />
                  </div>
