...
import {
  ArrowLeft, Server, AlertTriangle, AlignLeft, Calendar, MemoryStick as MemoryStickIcon,
...
export default function CreateServer() {
  ...
  const [state, setState] = useState({
    name: '', desc: '', expiresAt: '', ram: 4, cpu: 150, disk: 10, ip: '', port: 25565, runtimeType: 'docker', 
        ...
        <nav className="sticky top-0 z-50 border-b border-[#232323] bg-[#050505]/90 backdrop-blur-md">
          <div className="max-w-3xl mx-auto px-5 h-16 flex items-center justify-between">
            ...
            <a href="#" onClick={(e) => { e.preventDefault(); navigate('/servers'); }} className="flex items-center gap-3 group">
              <span className="font-display font-bold text-lg tracking-wide">DTG <span className="text-[#8f8f8f] font-medium">PANEL</span></span>
            ...
            <div className={`${dir === 'forward' ? 'anim-forward' : 'anim-back'}`}>
              ...
              {currentStep === 0 && (
                <div className="step-content">
                  ...
                  <label className="flex items-center gap-2 text-sm text-[#8f8f8f] mb-2.5">
                    <Calendar className="w-4 h-4" /> Deployment Expiry <span className="text-[#4c4c4c]">(optional)</span>
                  </label>
                  <input
                    type="datetime-local"
                    className="inp mb-7"
