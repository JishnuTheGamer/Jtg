...
export default function Login() {
  ...
  const { 
    ...
    enableGoogleLogin, enableDiscordLogin, firebaseApiKey, firebaseAuthDomain, firebaseProjectId,
  ...
  useEffect(() => {
    ...
    if (token) {
      axios.get("/api/auth/me", { headers: { Authorization: `Bearer ${token}` } }).then((res) => {
        login(token, res.data.user);
        navigate("/", { replace: true });
      }).catch(() => setError("Social login session could not be verified."));
    } else if (params.get("error")) {
      setError(params.get("error") === "discord_not_configured" ? "Discord login is not configured by the administrator." : "Discord login failed. Please try again.");
    }
  }, []);
  ...
  return (
    <div className="desert-wrapper" onMouseMove={handleMouseMove}>
      ...
      <div className="login-card">
        ...
        {enableDiscordLogin && <button type="button" onClick={handleDiscordLogin} disabled={isLoading} className="mt-3 w-full rounded-xl border border-[#5865F2]/50 bg-[#5865F2]/15 px-4 py-3 font-semibold text-white transition hover:bg-[#5865F2]/25 disabled:opacity-50">
          <span className="mr-2">◈</span> Sign in with Discord
        </button>}
