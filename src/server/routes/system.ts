...
router.get("/database", async (req, res) => {
  ...
  try {
    ...
    for (const name of entries.filter((entry) => entry.endsWith(".json"))) {
      ...
      const value = await fs.readJson(filePath).catch(() => null);
      const records = Array.isArray(value) ? value.length : value && typeof value === "object" ? Object.keys(value).length : 0;
      const stat = await fs.stat(filePath);
      files.push({ name, records, size: stat.size });
    }
...
router.put("/settings", async (req, res) => {
  ...
  const { 
    ...
    enablePlayit, enableTutorial, enableLoginAnimation, enableRegistration, theme, customPrimary, customSecondary, customGradient,
    enableGoogleLogin, enableDiscordLogin, discordClientId, discordRedirectUri, firebaseApiKey, firebaseAuthDomain, firebaseProjectId,
    firebaseStorageBucket, firebaseMessagingSenderId, firebaseAppId, defaultRuntime 
  } = req.body;
  const settings = await readJSON("settings.json") || {};
  if (panelName !== undefined) {
    settings.panelName = panelName || "DTG PANEL";
    try {
      const fs = await import("fs/promises");
  ...
  if (blueprintMode !== undefined) settings.blueprintMode = Boolean(blueprintMode);
  if (blueprintOpacity !== undefined) settings.blueprintOpacity = Math.min(0.5, Math.max(0, Number(blueprintOpacity)));
  if (blueprintSize !== undefined) settings.blueprintSize = Math.min(160, Math.max(16, Number(blueprintSize)));
  ...
  if (customPrimary !== undefined) settings.customPrimary = String(customPrimary);
  if (customSecondary !== undefined) settings.customSecondary = String(customSecondary);
  if (customGradient !== undefined) settings.customGradient = String(customGradient);
  if (enableGoogleLogin !== undefined) settings.enableGoogleLogin = enableGoogleLogin;
  if (enableDiscordLogin !== undefined) settings.enableDiscordLogin = Boolean(enableDiscordLogin);
  if (discordClientId !== undefined) settings.discordClientId = String(discordClientId).trim();
  if (discordRedirectUri !== undefined) settings.discordRedirectUri = String(discordRedirectUri).trim();
  ...
  if (defaultRuntime !== undefined) {
    if (!["docker", "local"].includes(defaultRuntime)) {
      return res.status(400).json({ error: "Invalid runtime. Choose docker or local." });
    }
