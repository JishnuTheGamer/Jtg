...
export default function ModManager({ serverId, server }: ModManagerProps) {
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm mt-1 text-amber-300/90 leading-relaxed">
                  You are currently running <strong>{serverType}</strong>. Proxy networks (Velocity, BungeeCord, Waterfall) do not support Minecraft Forge or Fabric mods.
                </p>
              </div>
            </div>
          </div>
  ...
  // If not mod loader software (e.g. Paper/Spigot)
  if (!isCompatibleSoftware) {
    return (
      <div className="flex-1 overflow-y-auto p-4 md:p-8 text-foreground">
        <div className="max-w-4xl mx-auto space-y-6">
          <div className="p-6 rounded-2xl bg-blue-500/10 border border-blue-500/30 text-blue-300">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm mt-1 text-blue-300/90 leading-relaxed">
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Search and install Fabric & Forge mods from <strong>Modrinth</strong> with auto-version detection.
            </p>
          </div>
        ...
        <div className="p-3.5 rounded-xl bg-zinc-900/60 border border-zinc-800 flex flex-wrap items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2.5 flex-wrap">
            ...
            <span className="px-2 py-1 rounded bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 font-mono font-bold">
              {normalizedGameVer}
            </span>
            <span className="text-zinc-500">•</span>
            <span className="text-zinc-400">Target Loader:</span>
            <span className="text-zinc-300 font-medium capitalize">{serverType.toLowerCase()}</span>
          </div>
              ...
              <form onSubmit={handleSearchSubmit} className="flex flex-col sm:flex-row gap-2">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full bg-zinc-900 border border-zinc-800 rounded-xl py-2.5 pl-10 pr-4 text-sm text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-indigo-500 transition-colors"
                  />
                </div>
                <button
                  type="submit"
                  className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-sm font-semibold transition-colors shrink-0 shadow-sm flex items-center justify-center gap-2"
                >
            ...
            <div className="bg-zinc-950/70 border border-zinc-800 rounded-2xl shadow-lg overflow-hidden divide-y divide-zinc-800/70">
              {loading ? (
                <div className="p-12 text-center text-zinc-400 flex flex-col items-center">
                  <RefreshCw className="w-7 h-7 animate-spin mb-3 text-indigo-500" />
                  <span className="font-medium">Searching Modrinth mod catalog...</span>
                </div>
              ) : mods.length === 0 ? (
                <div className="p-12 text-center text-zinc-400 flex flex-col items-center">
                  <AlertCircle className="w-8 h-8 mb-3 text-zinc-500" />
                  <p className="font-semibold text-zinc-200">No mods found</p>
                  <p className="text-xs text-zinc-500 mt-1">
                    Try searching with another keyword like &quot;sodium&quot; or &quot;jei&quot;.
                  </p>
                </div>
              ) : (
                mods.map((mod) => {
                    ...
                    >
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {mod.author && (
                              <span className="text-xs text-zinc-500">
                                by <span className="text-zinc-400 font-medium">{mod.author}</span>
                              </span>
                            )}
                          </div>
                      ...
                      <div className="flex items-center gap-2 w-full md:w-auto shrink-0 pt-2 md:pt-0">
                        ...
                        <button
                          ...
                          className="flex-1 md:flex-initial px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-semibold transition-all flex items-center justify-center gap-1.5 shadow-sm disabled:opacity-50"
                        >
                          {isInstallingThis ? (
                            <>
              ...
              <div className="flex items-center justify-between pb-4 border-b border-zinc-800">
                <div>
                  <h3 className="text-base font-bold text-zinc-100 flex items-center gap-2">
                    ...
                    <span>Installed Mods in <code>mods/</code></span>
                  </h3>
                  <p className="text-xs text-zinc-400 mt-0.5">
                    Managing <strong>{installedMods.length}</strong> mod jar files.
                  </p>
                </div>
              ...
              <div className="divide-y divide-zinc-800/80 mt-2">
                {loadingInstalled ? (
                  <div className="p-8 text-center text-zinc-400 flex flex-col items-center">
                    ...
                    <span>Scanning mods directory...</span>
                  </div>
                ) : installedMods.length === 0 ? (
                  <div className="p-8 text-center text-zinc-400 flex flex-col items-center">
                    <Box className="w-8 h-8 mb-2 text-zinc-600" />
                  ...
                  installedMods.map((mod) => (
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        <div className="w-9 h-9 rounded-lg bg-zinc-900 border border-zinc-800 flex items-center justify-center shrink-0 text-indigo-400">
