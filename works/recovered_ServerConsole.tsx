...
export default function ServerConsole({ serverId, server }: ServerConsoleProps) {
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      ...
      setLogs((p) => [...p, "[System] Live terminal session established."]);
    });
    ...
    socket.on("disconnect", (reason: string) => {
      setLogs((p) => [...p, `[System] Terminal disconnected: ${reason}`]);
    });
  ...
  return (
    <div className="w-full h-full flex flex-col p-2 sm:p-4 min-h-[550px] font-sans">
      <div className="flex-1 flex flex-col qx-glass rounded-2xl border border-white/10 shadow-2xl overflow-hidden relative">
              ...
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-theme-400 font-mono text-[10px] sm:text-[11px] mt-0.5">
                {isOnline ? (
                  <>
                    ...
                    <span className="text-theme-500/40 hidden xs:inline">â€¢</span>
                ...
                {server?.port && (
                  <>
                    <span className="text-theme-500/40 hidden sm:inline">â€¢</span>
          ...
          <div className="flex items-center gap-2 ml-auto shrink-0">
            {isOnline ? (
              ...
              >
                ...
                <span className="text-theme-400/80 text-[10px] hidden xs:inline">â€¢ {uptime}</span>
              </div>
            ) : (
        ...
        {/* RESUME TAIL FLOATING BUTTON */}
        {!autoScroll && (
          <button
            onClick={scrollToBottom}
            className="absolute bottom-40 right-6 z-20 flex items-center gap-2 px-3.5 py-2 rounded-xl bg-theme-500 text-black font-mono font-bold text-xs sm:text-sm shadow-xl shadow-theme-500/30 hover:bg-theme-400 transition-all animate-bounce"
          >
    ...
    </div> »            #…÷/ø~  Ü öâ=	                            &       '       )       2       4       ;       O       T       V       Y       [       c       e       i       k       n       p       s       u       w       y       z       |              ï      e       £ç÷/ø~  Ü öâ=Œu´ÿ~         8›´ÿ~         —È´ÿ~         ÿ$ÿÿÿÿÿÿÿÿÿÿÿÿÿÿÿ$ÿÿ    Á>     päÀÿ~   Àÿ~  päÀÿ~  päÀÿ~  0       ”%      import React, { useEffect, useState } from "react"; 
